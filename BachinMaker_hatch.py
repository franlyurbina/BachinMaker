...
class Eggbot_Hatch( inkex.Effect ):
	...
	def recursivelyTraverseSvg( self, aNodeList,
		...
		for node in aNodeList:
			...
			#matNew =  matCurrent * inkex.Transform( node.get( "transform" ) ).matrix 
 
			matrix1 = [list(i) for i in inkex.Transform( node.get( "transform" ) ).matrix] 
			matrix2 = [list(i) for i in matCurrent]
			matNew = []
			products = []
